...
Our test suite is implemented such that the number of processes in the system N, the number of
failed processes f and the number of read and write operations M can be configured before starting the
...
As $N$ increases, the quorum size (majority required for read/write) increases. For $N=100$, a process must successfully exchange messages with at least 51 processes (versus 2 processes for $N=3$). This introduces higher network traffic and processing overhead.
Comparing the $M=100$ case:
\begin{itemize}
    \item $N=3$: 113 ms
    \item $N=100$: 1343 ms
\end{itemize}
