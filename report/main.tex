...
A constraint to be taken into consideration is that put and get operations will always be invoked
on the same key k = 0. This is done in order to simplify the implementation.
...
from a quorum, the operation selects the value $v_m$ equipped with the highest timestamp $t_m$. If there are
multiple different values written with timestamp $t_m$, the largest such value is chosen. To make sure
that subsequent read operations will not “miss” the returned value, the reader then sends a write request
[$v_m$, $t_m$], waits until a quorum conﬁrms it by sending [ack, $v_m$, $t_m$], and only then returns v.
...
The behaviour of the actors is defined by event handlers, that being, the acions taken are event
driven. When a message is received, the corresponding handler is executed. It is not a good ideea
...
Notice that every read operation can be ordered in the history of the system, right after it's corresponding
write operation. This is because it's output is timestamped, so the history will reflect the one
of an atomic register.

The only way to violate the sequential specification would be to reverse the order of a write and
read operation. Assuming that a read operation precedes a write operation, it's timestamp must
be less then then write operation timestamp, therefore the read cannot return the value written
...
However, logging introduces overhead in terms of tens of tens of miliseconds per total run when
...
From the point of view of performance, the implementation shows good scalability characteristics.
From $N=3$ to $N=100$ while maintaining correctness in the presence of $f < N/2$ failures, the
performance penalty is acceptable for the increased fault tolerance. The latency inevitably grows
with $N$ due to the mechanics of the ABD-like majority consensus, the system remains responsive.
The experiments confirm that the protocol correctly handles the trade-off between fault
tolerance and performance latency.
